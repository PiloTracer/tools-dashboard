...
Traverse the entire codebase, identify all features, and create standardized documentation under `.work/features/`.
...
**`.work/features/{feature-name}/README.md`**
...
**`.work/features/{feature-name}/feature.yaml`**
...
**`.work/FEATURE_STANDARD.md`** - Update if needed to match project conventions

**`.work/FEATURE_DOCUMENTATION_SUMMARY.md`** - Update with actual features found

### 6. Update Agent Files

Check each agent file in `.work/agents/` and ensure they have:

```markdown
## Feature Documentation

When working on a feature, ALWAYS check `.work/features/{feature-name}/` first:
...
Ensure `.work/DIRECTORY_MAP.md` accurately reflects the project's `.work/` structure.

## Output Requirements

1. Create all feature directories under `.work/features/`
...
- Only create/update documentation in `.work/`
...
Verify the output:
1. Check `.work/features/` has all expected features
