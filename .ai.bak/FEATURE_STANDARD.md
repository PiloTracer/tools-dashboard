...
This document defines the standardized structure for feature documentation in the Tools Dashboard project.

## Why Centralized Feature Documentation?

Features in the Tools Dashboard are implemented across multiple services (front-public, front-admin, back-api, back-auth, shared contracts, etc.). Rather than keeping feature documentation scattered across each service, we maintain a **single source of truth** in `.ai/features/` that maps out where a feature is implemented across the entire codebase.
...
links:
  ...
  prompts:
    - .ai/prompts/{feature-name}-starter.md
  plans:
...
1. Create `.ai/features/{feature-name}/README.md` with business description
2. Create `.ai/features/{feature-name}/feature.yaml` with complete path mapping
