...
Traverse the entire codebase, identify all features, and create standardized documentation under `.ai/features/`.
...
**`.ai/features/{feature-name}/README.md`**
...
**`.ai/features/{feature-name}/feature.yaml`**
...
**`.ai/FEATURE_STANDARD.md`** - Update if needed to match project conventions

**`.ai/FEATURE_DOCUMENTATION_SUMMARY.md`** - Update with actual features found

### 6. Update Agent Files

Check each agent file in `.ai/agents/` and ensure they have:

```markdown
## Feature Documentation

When working on a feature, ALWAYS check `.ai/features/{feature-name}/` first:
...
Ensure `.ai/DIRECTORY_MAP.md` accurately reflects the project's `.ai/` structure.

## Output Requirements

1. Create all feature directories under `.ai/features/`
...
- Only create/update documentation in `.ai/`
...
Verify the output:
1. Check `.ai/features/` has all expected features
