...
**Repository alignment (April 2026):** User-facing OAuth route modules are registered from `front-public/app/routes/` (`oauth.authorize.tsx`, `oauth.token.tsx`, `oauth.revoke.tsx`, `.well-known.*`). Internal token work is exposed on `back-auth` under `/internal/oauth/*` (see `back-auth/features/auto-auth/feature.yaml`). Admin client tooling lives under `front-admin/app/features/auto-auth/`. Treat this narrative spec as architecture guidance; `.claude/features/auto-auth/feature.yaml` tracks file paths against the tree.
