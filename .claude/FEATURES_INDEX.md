...
Complete index of all documented features in the Tools Dashboard project. Each feature has a corresponding directory in `.claude/features/` with standardized documentation.
...
**Directory:** `.claude/features/auto-auth/`
...
**Directory:** `.claude/features/email-auth/`
...
**Directory:** `.claude/features/google-auth/`

Google sign-in for end users is wired through **user-registration** (front-public routes + `back-auth` provider callbacks). `google-auth` on `back-auth` currently exposes a small `/auth/google/start` helper—read `.claude/features/google-auth/feature.yaml` for the exact surface.
...
**Directory:** `.claude/features/two-factor/`
...
### 5. User Logout
**Directory:** `.claude/features/user-logout/`
...
### 6. User Registration
**Directory:** `.claude/features/user-registration/`
...
**Directory:** `.claude/features/user-management/`
...
**Directory:** `.claude/features/user-status/`
...
**Directory:** `.claude/features/progressive-profiling/`
...
**Directory:** `.claude/features/user-subscription/`
...
**Directory:** `.claude/features/change-language/`
...
### 12. Admin Sign-In
**Directory:** `.claude/features/admin-signin/`
...
### 13. Task Scheduler
**Directory:** `.claude/features/task-scheduler/`
...
**Directory:** `.claude/features/app-library/`
...
These are **separate deployables** or **API slices** not previously indexed next to product features. Each has `.claude/features/{name}/`.

### 15. Users OAuth resource (`/users/me`)
**Directory:** `.claude/features/users-oauth-resource/`
...
### 16. Feature registry
**Directory:** `.claude/features/feature-registry/`
...
**Directory:** `.claude/features/websockets/`
...
**Directory:** `.claude/features/background-workers/`
...
4. Look for related prompts/plans in `.claude/prompts/` and `.claude/plans/`
...
> Counts above are high-level planning figures; several features are still partial scaffolds—always prefer the per-feature `feature.yaml` under `.claude/features/{name}/` plus the service-local YAML in `back-*` / `front-*`.
...
- **Convention Guide:** `.claude/CONVENTIONS.md` - Code conventions and standards
- **Directory Map:** `.claude/DIRECTORY_MAP.md` - Project structure overview
