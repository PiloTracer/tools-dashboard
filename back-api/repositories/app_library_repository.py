...
class AuditLogRepository:
    ...
    async def create(
        ...
        # Helper to convert UUID to string for JSON serialization
        ...
        changes_json = json.dumps(changes, default=default_serializer) if changes else None
        snapshot_json = json.dumps(snapshot, default=default_serializer) if snapshot else None
        ...
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                """,
                app_uuid, event_type, performed_by, changes_json, snapshot_json, ip_address, user_agent
            )
            return dict(row) if row else {}
