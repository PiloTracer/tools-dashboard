...
**Agnostic** process (skills, scripts, templates) resolves from **`$SOC_SOURCE`**
(`/mnt/work/Projects/pilo.ai.soc.logicbison`) in thin-client mode — this repo has **no** local `.ai.soc/`.
...
Configured in `.cursorrules` § Security OS:
...
Operator entry: `$SOC_SOURCE/START_HERE.md`
...
```bash
bash $SOC_SOURCE/templates/bootstrap.sh
```
